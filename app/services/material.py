...
# PINGOO_STYLE_MATERIAL_ROUTING_V2
def _pingoo_is_animation_term(search_term: str) -> bool:
    """
    Detect visual-style searches generated by Pingoo.

    When Pixabay is configured, cartoon / illustration / 3D
    searches can use its animation catalogue instead of forcing
    Pexels stock footage to behave like an animation library.
    """
...
def search_videos_pexels(
    ...
    try:
        ...
        for v in videos:
            ...
            # Select the nearest Full-HD-or-better rendition.
            # Exact 1080x1920 is preferred when available.
            # Higher-resolution files remain eligible so a relevant
            # source is not discarded only because Pexels omitted the
            # exact target rendition.
            candidates = []
...
def search_videos_pixabay(
    ...
    params = {
        ...
        "video_type": (
            "animation"
            if _pingoo_is_animation_term(search_term)
            else "all"
        ),
        "per_page": 50,
        "key": api_key,
...
def download_videos(
    ...
    def search_videos(
        ...
        # Cartoon / illustration / 3D:
        # prefer Pixabay animation catalogue when a key exists.
        # Never break generation if Pixabay is unavailable;
        # Pexels remains the automatic fallback.
        if (
            provider == "pexels"
            and _pingoo_is_animation_term(search_term)
            and config.app.get("pixabay_api_keys")
        ):
            logger.info(
                "Pingoo animation search: "
                "trying Pixabay animation catalogue"
            )

            animation_items = _search_videos_with_cache(
                provider="pixabay",
            ...
            logger.warning(
                "Pixabay animation returned no usable videos; "
